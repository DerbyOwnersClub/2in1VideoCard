...
class BorderlessVideoWindow(Gtk.Window):
    def __init__(self):
        ...
        pipeline_str = f"""
        compositor name=comp background=transparent sink_0::xpos=0 sink_1::xpos=640 ! \
            video/x-raw,width=1300,height=720 ! \
            videoconvert ! {sink_desc} \
        v4l2src device={video_device1} io-mode=2 do-timestamp=true ! \
            image/jpeg,framerate=60/1,width=1920,height=1080 ! jpegdec ! \
            videocrop name=crop1 left=20 right=35 top=0 bottom=0 ! \
