...
class BorderlessVideoWindow(Gtk.Window):
    def __init__(self):
        ...
        # Build pipeline string based on chosen sink
        ...
        compositor name=comp latency=0 sink_1::xpos=640 sink_1::ypos=0 background=black ! \
            videoconvert ! glimagesink name=vsink sync=false max-lateness=0 \
        v4l2src device={video_device1} ! image/jpeg,framerate=30/1 ! jpegdec ! videoscale ! \
            video/x-raw,width=640,height=720 ! comp. \
        v4l2src device={video_device2} ! image/jpeg,framerate=30/1 ! jpegdec ! videoscale ! \
            video/x-raw,width=640,height=720 ! comp.
        """
        ...
        # If gtksink: grab its GTK widget and pack it. No set_window_handle needed.
        if self.sink_kind == "gtksink":
            try:
                ...
                sink_widget = None
            if sink_widget:
                # Replace box contents with the sink's widget
                for child in self.box.get_children():
                    self.box.remove(child)
                self.box.pack_start(sink_widget, True, True, 0)
                self.show_all()
                # Optional: tweak aspect ratio if supported
                try:
            ...
            # glimagesink/ximagesink: embed via DrawingArea + set_window_handle BEFORE PLAYING
            self._embed_with_handle()

    def _embed_with_handle(self):
        # Create DrawingArea and pack it
        ...
        # Try to set useful sink properties if available
        for prop, val in (("force-aspect-ratio", False),):
            try:
                self.vsink.set_property(prop, val)
            except Exception:
                pass

        # Start in READY so the sink doesn't create its own window yet
        self.pipeline.set_state(Gst.State.READY)

        # When the widget has an XID, pass it to the sink, THEN go PLAYING
        self.drawing_area.connect("realize", self.on_realize)

        # Also handle prepare-window-handle just in case
    ...
    def on_realize(self, widget):
        ...
        if window and self.vsink:
            ...
            try:
                ...
                # Some sinks use different method names; ignore if unavailable
                pass
            # Now that handle is set, start the pipeline
    ...
    def on_sync_message(self, bus, message):
        ...
        if s.get_name() == "prepare-window-handle":
            # Provide handle ASAP to avoid the sink creating its own window
