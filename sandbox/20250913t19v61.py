...
"""
            pipeline_str = f"""
            compositor name=comp latency=0 background=transparent ! \
                video/x-raw,format=RGBA,width={self.base_w},height={self.base_h},framerate=30/1 ! \
                videoconvert ! {sink_desc} \
            v4l2src device={video_device1} io-mode=2 do-timestamp=true ! \
                image/jpeg,framerate=60/1 ! jpegdec ! \
                videoscale ! video/x-raw,width={self.base_w},height={self.base_h},framerate=30/1 ! \
                videoconvert ! video/x-raw,format=RGBA ! \
                videocrop name=crop1 top=0 bottom=0 left=0 right=0 ! \
                queue ! comp.sink_0 \
            v4l2src device={video_device2} io-mode=2 do-timestamp=true ! \
                image/jpeg,framerate=60/1 ! jpegdec ! \
                videoscale ! video/x-raw,width={self.base_w},height={self.base_h},framerate=30/1 ! \
                videoconvert ! video/x-raw,format=RGBA ! \
                videocrop name=crop2 top=0 bottom=0 left=0 right=0 ! \
                queue ! comp.sink_1
            """
"""
