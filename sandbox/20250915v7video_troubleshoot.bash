...
for dev in $DEVICES; do
  ...
  for res in 1920x1080 1280x720 640x480; do
    ...
    if [[ "$answer" =~ ^[Yy]$ ]]; then
      DEVICE_WORKED="yes"
	echo "$dev at $res MJPEG 60fps operater answered yes" | tee -a "$LOGFILE"
    fi
    ...
    if [[ "$answer" =~ ^[Yy]$ ]]; then
      DEVICE_WORKED="yes"
	echo "$dev at $res MJPEG 30fps operater answered yes" | tee -a "$LOGFILE"
    fi
  done
  ...
  read -n 1 -r -p "Did you see video for $dev at $res YUYV 10fps? (y/n): " answer
  echo
  if [[ "$answer" =~ ^[Yy]$ ]]; then
    DEVICE_WORKED="yes"
	echo "video for $dev at $res YUYV 10fps operater answered yes" | tee -a "$LOGFILE"
  fi

  ## GStreamer JPEGDEC test
  echo "--- GStreamer JPEGDEC test ---" | tee -a "$LOGFILE"
  timeout 5 gst-launch-1.0 v4l2src device="$dev" ! jpegdec ! fakesink \
    >>"$LOGFILE" 2>&1

read -n 1 -r -p "Did you see video for $dev $res GStreamer JPEGDEC? (y/n): " answer y echo

  if [[ "$answer" =~ ^[Yy]$ ]]; then
    DEVICE_WORKED="yes"
	echo "video for $dev $res GStreamer JPEGDEC operator answered yes" | tee -a "$LOGFILE"
  fi
...
#  if [[ "$answer" =~ ^[Yy]$ ]]; then
#   DEVICE_WORKED="yes"
#	echo "$dev MJPEG $res operater answered yes" | tee -a "$LOGFILE"
# fi
