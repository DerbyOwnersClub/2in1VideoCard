...
class BorderlessVideoWindow(Gtk.Window):
    def __init__(self):
        ...
        compositor name=comp latency=0 background=transparent ! \
            video/x-raw,format=RGBA,width={WINDOW_WIDTH},height={WINDOW_HEIGHT},framerate=60/1 ! \
            videoconvert ! {sink_desc} \

        v4l2src device={video_device1} io-mode=2 do-timestamp=true ! \
            image/jpeg,framerate=60/1 ! jpegdec ! \
            videoscale ! video/x-raw,width={WINDOW_WIDTH},height={WINDOW_HEIGHT},framerate=60/1 ! \
        ...
        v4l2src device={video_device2} io-mode=2 do-timestamp=true ! \
            ...
            videoscale ! video/x-raw,width={WINDOW_WIDTH},height={WINDOW_HEIGHT},framerate=60/1 ! \
            videoconvert ! video/x-raw,format=RGBA ! \
            videocrop name=crop2 top=0 bottom=0 left=40 right=0 ! \
            queue ! comp.sink_1
...
#        self.pad2.set_property("height", int(self.base_h * z))
    ...
    def refresh_pipeline(self):
        try:
            ...
            pipeline_str = f"""gst-launch-1.0   compositor name=comp sink_0::xpos=0 sink_1::xpos=1827 latency=0 background=transparent  !  xvimagesink sync=false   v4l2src device=/dev/video2 ! image/jpeg,width=1920,height=1080,framerate=30/1 ! jpegdec ! videocrop left=40 right=53 ! comp.   v4l2src device=/dev/video4 ! image/jpeg,width=1920,height=1080,framerate=30/1 ! jpegdec ! videocrop left=44 right=52 ! comp.
"""
